...
- (void)layoutSubviews
{
    ...
    // 우리 앱은 광고 제거 구매시 높이가 더 내려간다. 오프셋을 따로 조정할 필요가 있을 듯(폰에서만)
    NSInteger offsetForNoAds = 0;
	...
	switch( [UIDevice currentDevice].orientation )
	{
		case UIDeviceOrientationPortraitUpsideDown:
			...
			y = JLTOAST_OFFSET_PORTRAIT_Y - offsetForNoAds;
			angle = M_PI;
			break;
		...
		case UIDeviceOrientationLandscapeLeft:
			...
			x = JLTOAST_OFFSET_LANDSCAPE_Y - offsetForNoAds;
		...
		case UIDeviceOrientationLandscapeRight:
			width = _backgroundView.frame.size.height;
			height = _backgroundView.frame.size.width;
			x = [UIScreen mainScreen].bounds.size.width - width - JLTOAST_OFFSET_LANDSCAPE_Y + offsetForNoAds;
			y = ([UIScreen mainScreen].bounds.size.height - height) / 2;
			angle = -M_PI_2;
			break;
		...
		default:
			...
			height = _backgroundView.frame.size.height;
			x = ([UIScreen mainScreen].bounds.size.width - width) / 2;
			y = [UIScreen mainScreen].bounds.size.height - height - JLTOAST_OFFSET_PORTRAIT_Y + offsetForNoAds;
			angle = 0;
			break;
